...
Write-Host "`n[CONFIG] Enabling .htaccess Support in Apache..." -ForegroundColor Cyan
...
Write-Host "`n[CHECK] Checking current AllowOverride settings..." -ForegroundColor Yellow
...
for ($i = 0; $i -lt $lines.Count; $i++) {
    if ($lines[$i] -match '<Directory "C:/xampp/htdocs">') {
        ...
        for ($j = $i; $j -lt [Math]::Min($i + 20, $lines.Count); $j++) {
            if ($lines[$j] -match '^\s*AllowOverride\s+(.+)') {
                ...
                if ($currentValue -eq "All") {
                    Write-Host "  [OK] AllowOverride already set to All" -ForegroundColor Green
                    $found = $true
                    break
                } else {
                    Write-Host "  [CHANGE] Changing to AllowOverride All" -ForegroundColor Yellow
        ...
        if (-not $found) {
            Write-Host "  [WARN] No AllowOverride found, adding it" -ForegroundColor Yellow
...
Write-Host "`n[SAVE] Saving changes..." -ForegroundColor Yellow
$lines | Set-Content $httpdConf -Encoding UTF8
Write-Host "  [OK] httpd.conf updated" -ForegroundColor Green

# Also check for LoadModule rewrite_module
Write-Host "`n[CHECK] Checking mod_rewrite..." -ForegroundColor Yellow
$rewriteLine = Get-Content $httpdConf | Select-String "LoadModule rewrite_module"
if ($rewriteLine -match "^#") {
    Write-Host "  [CHANGE] mod_rewrite is commented out, enabling..." -ForegroundColor Yellow
    $content = Get-Content $httpdConf -Raw
    ...
    Write-Host "  [OK] mod_rewrite already enabled" -ForegroundColor Green
}

Write-Host "`n" + ("=" * 60) -ForegroundColor Gray
Write-Host "[SUCCESS] Configuration Updated!" -ForegroundColor Green
Write-Host "`n[IMPORTANT] You must restart Apache for changes to take effect:" -ForegroundColor Yellow
