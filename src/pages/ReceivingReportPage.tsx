...
export default function ReceivingReportPage() {
  ...
  useEffect(() => {
    if (id) {
      fetchPOAndDeliveries();
    }
  }, [id]);

  const fetchPOAndDeliveries = async () => {
    try {
      setLoading(true);
      
      // Fetch PO details first
      const poResponse = await fetch(`http://localhost:3001/api/purchase-orders/${id}`);
      if (!poResponse.ok) throw new Error('Failed to fetch purchase order');
      const poData = await poResponse.json();
      setPo(poData);

      // Fetch all deliveries for this PO
      const deliveriesResponse = await fetch(`http://localhost:3001/api/deliveries/by-po/${id}`);
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <Card className="w-full max-w-md border-red-200 bg-red-50">
          <CardContent className="pt-6 text-center">
            <p className="text-red-600 mb-4">{error || 'Purchase order not found'}</p>
  ...
  if (deliveries.length === 0) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <Card className="w-full max-w-md">
          <CardContent className="pt-6 text-center">
            <p className="text-slate-600 mb-4">No deliveries found for this purchase order.</p>
            <Button onClick={() => navigate(-1)}>
              <ArrowLeft className="w-4 h-4 mr-2" />
              Go Back
            </Button>
    ...
    <>
      ...
      <div className="min-h-screen bg-slate-100">
        ...
        <div className="no-print sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center shadow-sm z-10">
          ...
          <Button variant="default" onClick={handlePrint} className="bg-blue-600 hover:bg-blue-700">
        ...
        {/* Report Content */}
        <div className="receiving-report-container bg-white max-w-5xl mx-auto my-6 p-8 shadow-lg rounded-lg">
