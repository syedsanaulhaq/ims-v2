...
import { 
  ...
  const BranchDashboard = () => {
    ...
    const [stats, setStats] = useState({ totalRequests: 0, pendingRequests: 0, approvedRequests: 0, itemsInBranch: 0, totalMembers: 0 });
      ...
      const loadData = async () => {
        try {
          ...
          const [requestsRes, inventoryRes, membersRes] = await Promise.all([
            fetch('http://localhost:3001/api/branch-inventory/requests', { credentials: 'include' }).then((r) => (r.ok ? r.json() : { requests: [] })),
            fetch(`http://localhost:3001/api/branch-inventory/${branchParam}`, { credentials: 'include' }).then((r) => (r.ok ? r.json() : { items: [] })),
            fetch(`http://localhost:3001/api/ims/users/aspnet/filtered${memberQuery}`, { credentials: 'include' }).then((r) => (r.ok ? r.json() : [])),
          ]);

          const requests = Array.isArray(requestsRes?.requests) ? requestsRes.requests : [];
          const pendingRequests = requests.filter((r: any) => ['submitted', 'pending', 'under_review'].includes(String(r.current_status || '').toLowerCase())).length;
          const approvedRequests = requests.filter((r: any) => ['approved', 'completed'].includes(String(r.final_status || '').toLowerCase())).length;
        ...
        <div className="p-6 space-y-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <Badge variant="outline" className="bg-green-100 text-green-800 border-green-300 mt-3">Active</Badge>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <Card className="cursor-pointer hover:shadow-xl transition-all duration-300 bg-gradient-to-br from-orange-50 to-orange-100 border-l-4 border-l-orange-500" onClick={() => navigate('/dashboard/branch-request-history')}>
              <CardHeader className="pb-2">
                <CardTitle className="flex items-center gap-2 text-orange-700"><ClipboardList className="h-5 w-5" />Branch Requests</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card className="cursor-pointer hover:shadow-xl transition-all duration-300 bg-gradient-to-br from-blue-50 to-blue-100 border-l-4 border-l-blue-500" onClick={() => navigate('/dashboard/branch-inventory')}>
              <CardHeader className="pb-2">
                <CardTitle className="flex items-center gap-2 text-blue-700"><Package className="h-5 w-5" />Items in Branch</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card className="cursor-pointer hover:shadow-xl transition-all duration-300 bg-gradient-to-br from-purple-50 to-purple-100 border-l-4 border-l-purple-500" onClick={() => navigate('/dashboard/branch-request-history')}>
              <CardHeader className="pb-2">
                <CardTitle className="flex items-center gap-2 text-purple-700"><CheckCircle className="h-5 w-5" />Need Approval</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card className="cursor-pointer hover:shadow-xl transition-all duration-300 bg-gradient-to-br from-green-50 to-green-100 border-l-4 border-l-green-500" onClick={() => navigate('/dashboard/branch-members')}>
              <CardHeader className="pb-2">
                <CardTitle className="flex items-center gap-2 text-green-700"><Users className="h-5 w-5" />Branch Members</CardTitle>
              </CardHeader>
              <CardContent>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              ...
              <CardContent>
                ...
                <p>Approved/completed: {stats.approvedRequests}</p>
              </CardContent>
            </Card>

            <Card>
              <CardHeader><CardTitle>Branch Overview</CardTitle></CardHeader>
              <CardContent>
                <p>Issued items: {stats.itemsInBranch}</p>
