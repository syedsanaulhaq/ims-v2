-- ============================================================================
-- Production Fix: Missing Views + System Settings (SQL Server 2008+ compatible)
-- Uses DROP/CREATE VIEW instead of CREATE OR ALTER VIEW
-- Replaces STRING_AGG with FOR XML PATH
-- ============================================================================
...
-- ============================================================================
PRINT 'Ensuring system_settings columns...';
GO

IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'system_settings' AND COLUMN_NAME = 'setting_type')
BEGIN
  ALTER TABLE system_settings ADD setting_type NVARCHAR(50) NULL;
  PRINT 'Added setting_type column.';
END
ELSE
BEGIN
  PRINT 'setting_type column already exists.';
END

IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'system_settings' AND COLUMN_NAME = 'description')
BEGIN
  ALTER TABLE system_settings ADD description NVARCHAR(500) NULL;
...
IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'system_settings' AND COLUMN_NAME = 'updated_by')
BEGIN
  ALTER TABLE system_settings ADD updated_by NVARCHAR(450) NULL;
...
-- ============================================================================
-- HELPER: Drop view if exists
-- ============================================================================
IF OBJECT_ID('dbo.vw_User_with_designation', 'V') IS NOT NULL DROP VIEW dbo.vw_User_with_designation;
GO
...
-- ============================================================================
IF OBJECT_ID('dbo.vw_employee_branch', 'V') IS NOT NULL DROP VIEW dbo.vw_employee_branch;
GO

CREATE VIEW dbo.vw_employee_branch
AS
...
GO
PRINT 'Created vw_employee_branch';
GO

-- ============================================================================
-- 4. vw_my_issuance_requests
-- ============================================================================
IF OBJECT_ID('dbo.vw_my_issuance_requests', 'V') IS NOT NULL DROP VIEW dbo.vw_my_issuance_requests;
GO
...
LEFT JOIN AspNetUsers sup ON CONVERT(NVARCHAR(450), sir.supervisor_id) = CONVERT(NVARCHAR(450), sup.Id)
LEFT JOIN AspNetUsers adm ON CONVERT(NVARCHAR(450), sir.admin_id) = CONVERT(NVARCHAR(450), adm.Id);
GO
...
-- ============================================================================
IF OBJECT_ID('dbo.vw_pending_supervisor_approvals', 'V') IS NOT NULL DROP VIEW dbo.vw_pending_supervisor_approvals;
GO
...
SELECT sir.id AS request_id,
       ...
       STUFF((SELECT ', ' + sii2.nomenclature
              FROM dbo.stock_issuance_items sii2
              WHERE sii2.request_id = sir.id
              FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'), 1, 2, '') AS item_list
FROM dbo.stock_issuance_requests sir
LEFT JOIN dbo.AspNetUsers u ON CONVERT(NVARCHAR(450), sir.requester_user_id) = CONVERT(NVARCHAR(450), u.Id)
...
-- ============================================================================
IF OBJECT_ID('dbo.vw_pending_admin_approvals', 'V') IS NOT NULL DROP VIEW dbo.vw_pending_admin_approvals;
GO
...
-- ============================================================================
IF OBJECT_ID('dbo.vw_delivery_summary', 'V') IS NOT NULL DROP VIEW dbo.vw_delivery_summary;
GO

CREATE VIEW dbo.vw_delivery_summary AS
SELECT
    d.id AS delivery_id,
    d.delivery_number,
    d.delivery_date,
...
GO
PRINT 'Created vw_delivery_summary';
GO

-- ============================================================================
-- 8. vw_po_fulfillment_status
-- ============================================================================
IF OBJECT_ID('dbo.vw_po_fulfillment_status', 'V') IS NOT NULL DROP VIEW dbo.vw_po_fulfillment_status;
GO

CREATE VIEW dbo.vw_po_fulfillment_status AS
SELECT
    po.id AS po_id,
    po.po_number,
    po.po_date,
...
GO
PRINT 'Created vw_po_fulfillment_status';
GO

-- ============================================================================
-- 9. View_Issuance_Status
-- ============================================================================
IF OBJECT_ID('dbo.View_Issuance_Status', 'V') IS NOT NULL DROP VIEW dbo.View_Issuance_Status;
GO
...
-- ============================================================================
IF OBJECT_ID('dbo.View_IssuanceApprovalStatus', 'V') IS NOT NULL DROP VIEW dbo.View_IssuanceApprovalStatus;
GO

CREATE VIEW dbo.View_IssuanceApprovalStatus AS
SELECT
    si.Id as IssuanceId,
    si.IssuanceNumber,
    si.RequestedBy,
...
GO
PRINT 'Created View_IssuanceApprovalStatus';
GO

-- ============================================================================
-- 11. View_Pending_Inventory_Verifications
-- ============================================================================
IF OBJECT_ID('dbo.View_Pending_Inventory_Verifications', 'V') IS NOT NULL DROP VIEW dbo.View_Pending_Inventory_Verifications;
GO
...
-- ============================================================================
IF OBJECT_ID('dbo.View_stock_transactions_clean', 'V') IS NOT NULL DROP VIEW dbo.View_stock_transactions_clean;
GO

CREATE VIEW dbo.View_stock_transactions_clean AS
SELECT
    stc.id,
    stc.tender_id,
    t.reference_number,
...
GO
PRINT 'Created View_stock_transactions_clean';
GO

-- ============================================================================
-- 13. View_tenders
-- ============================================================================
IF OBJECT_ID('dbo.View_tenders', 'V') IS NOT NULL DROP VIEW dbo.View_tenders;
GO

CREATE VIEW dbo.View_tenders
AS
...
GO
PRINT 'Created View_tenders';
GO

-- ============================================================================
-- 14. vw_item_masters_with_categories
-- ============================================================================
IF OBJECT_ID('dbo.vw_item_masters_with_categories', 'V') IS NOT NULL DROP VIEW dbo.vw_item_masters_with_categories;
GO

CREATE VIEW dbo.vw_item_masters_with_categories
AS
...
GO
PRINT 'Created vw_item_masters_with_categories';
GO

-- ============================================================================
-- 15. vw_ims_user_permissions
-- ============================================================================
IF OBJECT_ID('dbo.vw_ims_user_permissions', 'V') IS NOT NULL DROP VIEW dbo.vw_ims_user_permissions;
GO
...
-- ============================================================================
IF OBJECT_ID('dbo.vw_ims_user_roles_detail', 'V') IS NOT NULL DROP VIEW dbo.vw_ims_user_roles_detail;
GO
